...
class TeacherProfileViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            # Ensure institute_id is numeric to avoid 404 with string "undefined"
            if not str(institute_id).isdigit():
                return Response({'error': f'Invalid institute ID: "{institute_id}". Please ensure an institute is selected.'}, status=400)
            
            institute = get_object_or_404(Institute, id=institute_id, owner=request.user)
        except Exception:
            # Check if user has ANY institutes
            if not Institute.objects.filter(owner=request.user).exists():
                return Response({'error': 'You have no institutes created. Please create an institute first.'}, status=400)
            return Response({'error': f'Institute with ID "{institute_id}" not found or not owned by you.'}, status=404)
    ...
    def assign_students(self, request, pk=None):
        ...
        if not isinstance(student_ids, list):
            return Response({'error': 'student_ids must be a list'}, status=400)
            
        # Clear existing assignments or just add new ones? 
        # Usually 'assign' means 'set the list'.
        students = InstituteStudentProfile.objects.filter(id__in=student_ids, institute=teacher.institute)
        teacher.assigned_students.set(students)
        
        return Response({
            'message': f'Successfully assigned {students.count()} students to {teacher.user.username}',
            'assigned_students': TeacherProfileSerializer(teacher).data['assigned_students']
        })
...
class InstituteStudentProfileViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.persona == 'INSTITUTE_OWNER':
            ...
            teacher_profile = TeacherProfile.objects.filter(user=user, is_active=True).first()
            if teacher_profile:
                return teacher_profile.assigned_students.all()
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            # Ensure institute_id is numeric
            if not str(institute_id).isdigit():
                return Response({'error': f'Invalid institute ID: "{institute_id}". Please ensure an institute is selected.'}, status=400)
        ...
        # Try finding user by clean name or generated name
        base_username = clean_name
        suffix_username = f"{clean_name}_{parent_mobile[-4:]}"
        
        # Strategy: 
        # 1. If base_username (e.g. 'vaidhy') exists and is already a STUDENT, use it.
        # 2. Else if suffix_username exists, use it.
        # 3. Else create new user with suffix_username (safest for uniqueness).
        
        user = User.objects.filter(username=base_username).first()
        created = False
        
        if not user:
            user = User.objects.filter(username=suffix_username).first()
            if not user:
                user = User.objects.create_user(
                    username=suffix_username,
                    email=f"{suffix_username}@ibet.com",
                    password='student123',
                    persona='STUDENT',
        ...
        # Ensure persona is at least STUDENT if we are linking
        if user.persona not in ['STUDENT', 'STUDENT_ACADEMIC']:
        ...
        return Response({
            ...
            'temporary_password': 'student123' if created else 'Existing account linked',
    ...
    def get(self, request):
        ...
        # TEACHER ROLE
            ...
            if not profile:
                return Response({
                    'role': 'TEACHER',
                    'error': 'No teacher profile found. Please contact the institute admin to get set up.',
                    'profile': None,
                    'institutes': [],
                    'recent_salaries': [],
                    'student_fee_status': {},
