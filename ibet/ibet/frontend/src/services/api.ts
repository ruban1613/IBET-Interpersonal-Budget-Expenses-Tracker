...
class ApiService {
  ...
  async deposit(module: string, amount: number, otp: string, cacheKey?: string): Promise<any> {
    return this.request(`/${module}/wallet/deposit/`, {
      method: 'POST',
      body: JSON.stringify({ amount, otp, cache_key: cacheKey }),
    });
  }

  async withdraw(module: string, amount: number, otp: string, cacheKey?: string): Promise<any> {
...
async assignStudentsToTeacher(teacherId: number, studentIds: number[]): Promise<any> {
  return this.request(`/institute/teachers/${teacherId}/assign_students/`, {
    method: 'POST',
