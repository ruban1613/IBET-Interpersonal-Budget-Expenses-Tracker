...
export default function Dashboard() {
  ...
  const handleRequestOTP = async () => {
    ...
    setError('');
    try {
      const res = await api.requestOTP(module, parseFloat(amount));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (action === 'deposit') {
        await api.deposit(module, parseFloat(amount), otp, cacheKey);
        setSuccess('Deposit successful!');
      } else {
        await api.withdraw(module, parseFloat(amount), otp, cacheKey);
