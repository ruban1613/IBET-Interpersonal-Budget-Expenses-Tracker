    ...
    <div className="dashboard-container">
      <header className="dashboard-header glass">
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <button onClick={handleBack} className="back-btn" style={{ background: 'rgba(255,255,255,0.3)', border: '1px solid rgba(255,255,255,0.3)', color: 'var(--text-main)' }}>← Back</button>
          <button onClick={() => setShowNotifications(!showNotifications)} style={{ position: 'relative', background: 'transparent', border: 'none', fontSize: '1.3rem', cursor: 'pointer' }}>
            🔔 {alerts.filter(a => a.status !== 'READ').length > 0 && <span className="notification-badge" style={{ background: '#e74c3c', color: 'white', borderRadius: '50%', padding: '2px 6px', fontSize: '0.7rem', position: 'absolute', top: '-5px', right: '-5px' }}>{alerts.filter(a => a.status !== 'READ').length}</span>}
          </button>
        </div>
        <h1 style={{ color: 'var(--primary)' }}>Parent Dashboard 👨‍👩‍👧 {userProfile?.uid && <span style={{ fontSize: '0.9rem', color: 'white', background: 'rgba(255,255,255,0.2)', backdropFilter: 'blur(4px)', padding: '2px 8px', borderRadius: '4px', marginLeft: '10px', verticalAlign: 'middle' }}>{userProfile.uid}</span>}</h1>
      ...
      {showNotifications && (
        <div className="notifications-panel glass">
          <div className="panel-header">
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <h3>Alerts & OTPs</h3>
              <button onClick={loadParentData} className="refresh-btn-small" style={{ background: 'none', border: 'none', color: 'var(--primary)', cursor: 'pointer', fontSize: '0.9rem' }}>🔄 Refresh</button>
            </div>
            <button onClick={() => setShowNotifications(false)}>✕</button>
          </div>
          <div className="panel-content">
            {alerts.length === 0 ? <p style={{ color: 'rgba(255,255,255,0.6)' }}>No alerts</p> : alerts.map(alert => (
              <div key={alert.id} className={`alert-item ${alert.status === 'READ' ? 'read' : 'unread'} glass`} onClick={() => handleMarkAlertRead(alert.id)} style={{ marginBottom: '10px', padding: '12px', borderRadius: '8px' }}>
                <div className="alert-type" style={{ fontWeight: 'bold' }}>{alert.alert_type}</div>
      ...
      <main className="dashboard-main">
        ...
        <div className="balance-card glass">
          <h2>💰 My Wallet</h2>
          <div className="info-value" style={{ fontSize: '2.5rem', fontWeight: '800', margin: '1rem 0' }}>₹{parentWallet?.balance ? parseFloat(String(parentWallet.balance)).toFixed(2) : '0.00'}</div>
          <div className="button-group-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem' }}>
            <button onClick={() => setShowDepositForm(true)} className="submit-btn" style={{ marginTop: 0 }}>💳 Add Funds</button>
            <button onClick={() => setShowExpenseModal(true)} className="submit-btn" style={{ background: 'rgba(230, 126, 34, 0.8)', marginTop: 0 }}>📝 Record Expense</button>
            <button onClick={() => { setStatementType('parent'); setShowStatementModal(true); }} className="submit-btn" style={{ background: 'rgba(118, 75, 162, 0.8)', marginTop: 0 }}>📄 My Statement</button>
          </div>
        </div>

        <div className="balance-card glass" style={{ padding: '2.5rem' }}>
          <h2 style={{ color: 'white', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <span style={{ fontSize: '1.5rem' }}>👨‍👩‍👧‍👦</span> Linked Children
          </h2>
          <div className="info-cards" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
            {linkedStudents.map(s => (
              <div 
                key={s.id} 
                className={`info-card glass ${selectedStudent?.student_id === s.id ? 'selected' : ''}`} 
                onClick={() => handleSelectStudent(s.id)}
                style={{ 
                  position: 'relative', 
                  cursor: 'pointer',
                  border: selectedStudent?.student_id === s.id ? '2px solid rgba(255,255,255,0.8)' : '1px solid rgba(255,255,255,0.2)',
                  background: selectedStudent?.student_id === s.id ? 'rgba(255,255,255,0.3)' : 'rgba(255,255,255,0.1)',
              ...
              >
                <div style={{ position: 'absolute', top: 0, right: 0, padding: '8px' }}>
                  <button 
                    onClick={(e) => { e.stopPropagation(); handleUnlinkStudent(s.id); }} 
                    style={{ 
                ...
                <h3 style={{ margin: 0, color: 'white', fontSize: '1.1rem' }}>{s.username}</h3>
                <div className="info-value" style={{ fontSize: '1.5rem', marginTop: '0.5rem', color: '#90EE90' }}>₹{parseFloat(String(s.wallet_balance)).toFixed(2)}</div>
                <div style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.6)', marginTop: '4px' }}>Click to view details</div>
              </div>
            ))}
            
            <button 
              className="info-card add-card glass pulse" 
          ...
          <>
            <div className="balance-card glass">
              ...
              <div className="info-cards">
                <div className="info-card glass"><h3>Wallet</h3><div className="info-value">₹{parseFloat(String(selectedStudent.wallet_balance)).toFixed(2)}</div></div>
                <div className="info-card glass"><h3>Spent Today</h3><div className="info-value" style={{ color: '#ff8a80' }}>₹{parseFloat(String(selectedStudent.current_daily_spent)).toFixed(2)}</div></div>
              </div>
              <div className="button-group-grid" style={{ marginTop: '1.5rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem' }}>
                <button onClick={() => { setShowAllowanceForm(true); setMonthlyAmount(''); setDailyAllowance(''); setShowAllowanceOtpInput(false); }} className="submit-btn" style={{ marginTop: 0 }}>Set Allowance</button>
                <button onClick={() => setShowTransferForm(true)} className="submit-btn" style={{ background: 'rgba(46, 204, 113, 0.8)', marginTop: 0 }}>💰 Transfer</button>
                <button onClick={() => { setStatementType('student'); setShowStatementModal(true); }} className="submit-btn" style={{ background: 'rgba(118, 75, 162, 0.8)', marginTop: 0 }}>📄 Statement</button>
              </div>
            </div>

            {/* NEW: Academic Portal Section for Parents */}
            {instituteData && (
              <div className="balance-card glass" style={{ borderTop: '4px solid rgba(52, 152, 219, 0.5)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                  <h2 style={{ margin: 0 }}>Academic Portal 🏫</h2>
                  <span style={{ padding: '4px 12px', borderRadius: '20px', background: 'rgba(52, 152, 219, 0.2)', color: '#3498db', fontSize: '0.9rem', fontWeight: 'bold' }}>{instituteData.institute_name}</span>
                </div>
                ...
                <div className="info-cards">
                  ...
                  <div className="info-card glass" style={{ borderLeft: `5px solid ${instituteData.current_fee_status.status === 'PAID' ? '#2ecc71' : '#e74c3c'}` }}>
                    <h3>Status: {instituteData.current_fee_status.status}</h3>
                    <div className="info-value" style={{ color: instituteData.current_fee_status.status === 'PAID' ? '#90EE90' : '#ff8a80' }}>
                    ...
                    {instituteData.current_fee_status.status !== 'PAID' && (
                      <button onClick={() => handlePayFee(instituteData.current_fee_status.id)} className="submit-btn" style={{ background: 'rgba(46, 204, 113, 0.8)', width: 'auto', padding: '0.4rem 1rem', fontSize: '0.85rem', marginTop: '5px' }}>💳 Pay Now</button>
                    )}
                  </div>
                  <div className="info-card glass">
                    <h3>Attendance</h3>
                    <div className="info-value" style={{ color: instituteData.attendance.percentage >= 75 ? '#90EE90' : '#ff8a80' }}>{instituteData.attendance.percentage}%</div>
                  ...
                  <div className="info-card glass">
                    ...
                    <div style={{ marginTop: '1rem', maxHeight: '200px', overflowY: 'auto' }}>
                      {instituteData.notifications.length === 0 ? <p style={{ opacity: 0.5 }}>No alerts</p> : instituteData.notifications.map((n: any) => (
                        <div key={n.id} style={{ padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.1)', fontSize: '0.8rem' }}>
                          <strong style={{ color: 'var(--primary)' }}>{n.notification_type.replace('_', ' ')}</strong>: {n.message}
                        </div>
                      ))}
                    </div>
                  </div>
                  <div className="info-card glass">
                    ...
                    <div style={{ marginTop: '1rem' }}>
                      {instituteData.attendance.recent.slice(0, 5).map((a: any) => (
                        <div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.1)', fontSize: '0.85rem' }}>
                          <span>{new Date(a.date).toLocaleDateString()}</span>
                          <span style={{ fontWeight: 'bold', color: a.status === 'PRESENT' ? '#90EE90' : '#ff8a80' }}>{a.status}</span>
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      {showStatementModal && (
        <div className="modal-overlay" onClick={() => setShowStatementModal(false)}>
          <div className="modal wide-modal glass" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2>{statementType === 'parent' ? 'My Statement 📜' : 'Child Statement 📜'}</h2>
              <button onClick={() => setShowStatementModal(false)} style={{ color: 'white' }}>✕</button>
            </div>
            <div className="filters-row" style={{ display: 'flex', gap: '10px', marginBottom: '1.5rem' }}>
              <select value={filterYear} onChange={e => setYear(parseInt(e.target.value))} style={{ background: 'rgba(255,255,255,0.2)', color: 'white' }}>{[2024, 2025, 2026].map(y => <option key={y} value={y} style={{ color: 'black' }}>{y}</option>)}</select>
              <select value={filterMonth} onChange={e => setMonth(parseInt(e.target.value))} style={{ background: 'rgba(255,255,255,0.2)', color: 'white' }}>{['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'].map((m, i) => <option key={m} value={i+1} style={{ color: 'black' }}>{m}</option>)}</select>
              <input type="number" placeholder="DD" value={filterDay} onChange={e => setDay(e.target.value)} min="1" max="31" style={{ background: 'rgba(255,255,255,0.2)', color: 'white' }} />
            </div>
                  ...
                  <tbody>
                    {statementData.map(tx => (
                      <tr key={tx.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                        ...
                        <td style={{textAlign:'right', fontWeight:'bold', color: tx.transaction_type === 'DEPOSIT' || tx.transaction_type === 'INC' ? '#90EE90' : '#ff8a80'}}>₹{parseFloat(tx.amount).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="modal-footer"><button onClick={() => window.print()} className="print-btn" style={{ background: 'rgba(255,255,255,0.2)', color: 'white', padding: '0.5rem 1.5rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.3)', cursor: 'pointer' }}>🖨️ Print</button></div>
          </div>
        </div>
      )}
          ...
          <div className="modal glass" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSetAllowance}>
              {!showAllowanceOtpInput ? (
                ...
                <>
                  <div className="otp-display glass" style={{ fontSize: '2rem', textAlign: 'center', padding: '1rem', background: 'rgba(255,255,255,0.2)', borderRadius: '12px', margin: '1rem 0' }}>{allowanceGeneratedOtp}</div>
                  <div className="form-group">
                    <label>Enter Verification OTP</label>
                    <input type="text" value={allowanceOtp} onChange={e => setAllowanceOtp(e.target.value)} required maxLength={6} style={{ textAlign: 'center', fontSize: '1.5rem', letterSpacing: '8px' }} />
                  </div>
          ...
          <div className="modal glass" onClick={e => e.stopPropagation()}>
            ...
            {!otpGenerated ? (
              ...
              <form onSubmit={handleTransferToStudent}>
                <div className="otp-display glass" style={{ fontSize: '2rem', textAlign: 'center', padding: '1rem', background: 'rgba(255,255,255,0.2)', borderRadius: '12px', margin: '1rem 0' }}>{generatedOtp}</div>
                <div className="form-group"><label>Enter OTP</label><input type="text" value={transferOtp} onChange={e => setTransferOtp(e.target.value)} required maxLength={6} style={{ textAlign: 'center', fontSize: '1.5rem', letterSpacing: '8px' }} /></div>
      ...
      {showUnlockOtp && (
        <div className="modal-overlay" onClick={() => setShowUnlockOtp(false)}>
          <div className="modal glass" style={{ textAlign: 'center' }} onClick={e => e.stopPropagation()}>
            <h2>🔑 Unlock OTP</h2>
            <p style={{ color: 'rgba(255,255,255,0.8)' }}>Share this with {unlockStudentName}:</p>
            <div className="otp-display-large glass" style={{ fontSize: '3rem', fontWeight: 'bold', margin: '1.5rem 0', color: 'white', background: 'rgba(255,255,255,0.2)', borderRadius: '16px', padding: '1rem' }}>{unlockOtpCode}</div>
