  ...
  return (
    <div className="dashboard-container">
      <header className="dashboard-header">
        ...
        <h1>Individual Dashboard 💼 {userProfile?.uid && <span style={{ fontSize: '0.9rem', color: 'white', background: 'rgba(255,255,255,0.2)', backdropFilter: 'blur(4px)', padding: '2px 8px', borderRadius: '4px', marginLeft: '10px', verticalAlign: 'middle' }}>{userProfile.uid}</span>}</h1>
      ...
      <main className="dashboard-main">
                      ...
                      if (active && payload && payload.length) {
                        ...
                        return (
                          <div className="glass" style={{ padding: '10px', borderRadius: '8px' }}>
                            <p style={{ margin: 0, fontWeight: 'bold', color: '#4f46e5' }}>{data.date}</p>
                            <p style={{ margin: '4px 0', fontSize: '1.1rem', fontWeight: 'bold' }}>₹{data.amount}</p>
                            <p style={{ margin: 0, color: 'var(--text-main)' }}>{data.category}: {data.description}</p>
                            <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '0.75rem' }}>Transaction #{data.index}</p>
                          </div>
                        );
                      }
                      return null;
                    ...
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="glass" style={{ padding: '10px', borderRadius: '8px' }}>
                          ...
                          <p style={{ margin: '4px 0', fontSize: '1.1rem', fontWeight: 'bold' }}>₹{data.amount}</p>
                          <p style={{ margin: 0, color: 'var(--text-main)' }}>{cat?.icon} {data.description}</p>
                          <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '0.8rem' }}>Category: {cat?.label || data.category}</p>
                        </div>
                      );
                    }
                    return null;
        ...
        {/* Savings Wallet Card */}
        <div className="balance-card special-wallet-card glass" style={{ background: 'linear-gradient(135deg, rgba(102, 126, 234, 0.7) 0%, rgba(118, 75, 162, 0.7) 100%)' }}>
          ...
          <div style={{ marginTop: '1rem', display: 'flex', gap: '0.5rem', justifyContent: 'center' }}>
            ...
            <button 
              ...
              className="action-btn" 
              style={{ flex: 1, background: 'rgba(255,255,255,0.2)', color: 'white', border: '1px solid rgba(255,255,255,0.3)' }}
            >
        ...
        <div className="transaction-card glass">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
            ...
            <button 
              ...
              className="submit-btn"
              style={{ background: 'rgba(118, 75, 162, 0.8)' }}
            >
              📄 View Statement
            </button>
            <button 
        ...
        {/* Recent Expenses */}
        <div className="transaction-history glass" style={{ marginTop: '2rem', padding: '1.5rem', borderRadius: '16px' }}>
          <h3 style={{ marginBottom: '1.5rem' }}>Today's Expenses</h3>
          {todaySpending?.expenses?.length === 0 ? (
            <div className="no-transactions" style={{ color: 'rgba(255,255,255,0.6)' }}>No expenses recorded today</div>
          ) : (
            <div className="transaction-list">
              {todaySpending?.expenses?.map(expense => (
                <div key={expense.id} className="transaction-item" style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.1)', marginBottom: '10px' }}>
                  <div className="transaction-info">
                    <span className="transaction-type" style={{ color: 'white' }}>
                      {EXPENSE_CATEGORIES.find(c => c.value === expense.category)?.icon || '📦'} {' '}
                      {EXPENSE_CATEGORIES.find(c => c.value === expense.category)?.label || expense.category}
                    </span>
                    {expense.description && (
                      <span style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.7)' }}>{expense.description}</span>
                    )}
                  </div>
                  <span className="transaction-amount debit" style={{ color: '#ff8a80' }}>₹{expense.amount}</span>
                </div>
              ))}
            </div>
          )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.5rem' }}>
              {monthlySummary.investment_suggestions.map(plan => (
                ...
                >
                  ...
                  <p style={{ color: '#555', fontSize: '0.95rem', marginBottom: '1rem', lineHeight: '1.4' }}>
                    {plan.description}
                  </p>
                  <div style={{ background: 'rgba(255,255,255,0.2)', padding: '1rem', borderRadius: '8px', marginBottom: '1rem' }}>
                    ...
                    <div style={{ fontSize: '0.9rem', color: '#555' }}>{plan.benefits}</div>
                  </div>
      ...
      {showStatementModal && (
        <div className="modal-overlay" onClick={() => setShowStatementModal(false)}>
          <div className="modal glass" style={{ maxWidth: '800px', width: '95%' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h2 style={{ margin: 0 }}>Transaction Statement 📜</h2>
              <button onClick={() => setShowStatementModal(false)} style={{ background: 'none', border: 'none', fontSize: '1.5rem', cursor: 'pointer', color: 'white' }}>✕</button>
            </div>
            ...
            <div style={{ 
              ...
              <div className="form-group" style={{ marginBottom: 0 }}>
                <label style={{ color: 'rgba(255,255,255,0.8)' }}>Year</label>
                <select 
                  value={filterYear} 
                  onChange={e => setYear(parseInt(e.target.value))}
                  style={{ background: 'rgba(255,255,255,0.2)', color: 'white', border: '1px solid rgba(255,255,255,0.2)' }}
                >
                  {[2024, 2025, 2026].map(y => <option key={y} value={y} style={{ color: 'black' }}>{y}</option>)}
                </select>
              </div>
              <div className="form-group" style={{ marginBottom: 0 }}>
                <label style={{ color: 'rgba(255,255,255,0.8)' }}>Month</label>
                <select 
                  value={filterMonth} 
              ...
              <div className="form-group" style={{ marginBottom: 0 }}>
                <label style={{ color: 'rgba(255,255,255,0.8)' }}>Specific Date (Optional)</label>
                <input 
                  type="number" 
                  min="1" 
                  max="31" 
            ...
            <div style={{ maxHeight: '400px', overflowY: 'auto', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px' }}>
              {loadingStatement ? (
                <div style={{ padding: '2rem', textAlign: 'center', color: 'white' }}>Loading transactions...</div>
              ) : statementData.length === 0 ? (
                <div style={{ padding: '2rem', textAlign: 'center', color: 'rgba(255,255,255,0.6)' }}>No transactions found for this period</div>
              ) : (
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', color: 'white' }}>
                  <thead style={{ position: 'sticky', top: 0, background: 'rgba(0,0,0,0.2)', backdropFilter: 'blur(8px)', zIndex: 1 }}>
                    <tr>
                      <th style={{ padding: '12px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>Date</th>
                      <th style={{ padding: '12px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>Description</th>
                      <th style={{ padding: '12px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>Type</th>
                      <th style={{ padding: '12px', borderBottom: '1px solid rgba(255,255,255,0.1)', textAlign: 'right' }}>Amount</th>
                    </tr>
                  </thead>
                  <tbody>
                    {statementData.map((tx: any) => (
                      <tr key={tx.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                        <td style={{ padding: '12px', fontSize: '0.9rem', color: 'rgba(255,255,255,0.8)' }}>
                          {new Date(tx.created_at).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
                        </td>
                        <td style={{ padding: '12px' }}>
                          <div style={{ fontWeight: '600' }}>{tx.description}</div>
                          <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.5)' }}>Balance after: ₹{parseFloat(tx.balance_after).toFixed(2)}</div>
                        </td>
                        <td style={{ padding: '12px' }}>
                          <span style={{ 
                            padding: '2px 8px', 
                        ...
                        <td style={{ padding: '12px', textAlign: 'right', fontWeight: 'bold', color: tx.transaction_type === 'DEPOSIT' ? '#90EE90' : '#ff8a80' }}>
            ...
            <div style={{ marginTop: '1.5rem', display: 'flex', justifyContent: 'flex-end' }}>
              <button 
                ...
                className="submit-btn" 
                style={{ width: 'auto', padding: '0.5rem 1.5rem', background: 'rgba(255,255,255,0.2)', color: 'white', border: '1px solid rgba(255,255,255,0.3)' }}
              >
                🖨️ Print Statement
              </button>
            </div>
