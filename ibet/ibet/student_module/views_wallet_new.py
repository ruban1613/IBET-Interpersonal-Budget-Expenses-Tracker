...
class StudentWalletViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['post'])
    def withdraw_special(self, request):
        """Special withdrawal from student special balance"""
        wallet = self.get_object()
        amount = Decimal(str(request.data.get('amount', 0)))
        description = request.data.get('description', _('Special Withdrawal'))
        
        if amount <= 0:
            return Response({'error': _('Invalid amount')}, status=status.HTTP_400_BAD_REQUEST)
        
        if wallet.special_balance < amount:
            return Response({'error': _('Insufficient special wallet balance')}, status=status.HTTP_400_BAD_REQUEST)
        
        with transaction.atomic():
            wallet.special_balance -= amount
            wallet.last_transaction_at = timezone.now()
            wallet.save()
        ...
        return Response({
            'message': _('Special withdrawal successful'),
            'new_special_balance': float(wallet.special_balance)
        })

    @action(detail=False, methods=['post'])
    def request_parent_approval(self, request):
        """Manually request parent approval for extra spending"""
        amount = Decimal(str(request.data.get('amount', 0)))
        description = request.data.get('description', _('Extra spending request'))
        student = request.user
        ...
        return Response({
            'status': 'pending_approval',
            'message': _('Request sent. Parent approval required.'),
            'otp_request_id': pending_request.id,
            'requested_amount': float(amount)
        })
    ...
    def daily_status(self, request):
        """Get status of today's spending and cumulative availability"""
        student = request.user
        today = timezone.localdate()
        ...
        try:
            ...
            total_available = tracker.total_available
            days_available = tracker.days_available
        except CumulativeSpendingTracker.DoesNotExist:
        ...
        try:
            da = DailyAllowance.objects.get(student=student, date=today)
            today_limit = da.daily_amount
            today_spent = da.amount_spent
            today_remaining = da.remaining_amount
            today_locked = da.is_locked
        except DailyAllowance.DoesNotExist:
            today_limit = Decimal('0.00')
            today_spent = Decimal('0.00')
            today_remaining = Decimal('0.00')
            today_locked = False
